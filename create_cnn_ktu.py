...
# load the train data
X = loadtxt('d:\\aud_collab\\EEGData_512_ktu.csv', delimiter=',')
print(X.shape)
...
input = rScaler.fit_transform(train_data[:, 0:1740])
testinput = rScaler.fit_transform(X_test_tmp[:,0:1740])
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=32, kernel_size=3, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), data_format='channels_last', padding='valid', activation='relu', strides=1, input_shape=(30, 58)))
model.add(Conv1D(filters=32, kernel_size=3, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), data_format='channels_last', padding='valid', activation='relu', strides=1))
model.add(Conv1D(filters=32, kernel_size=4, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), data_format='channels_last', padding='valid', activation='relu', strides=1))
...
mc = ModelCheckpoint('d:\\aud_collab\\best_model.h5', monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)
...
model.save("d:\\aud_collab\\model_conv1d.h5")

# load the saved model
saved_model = load_model('d:\\aud_collab\\best_model.h5')
# evaluate the model
